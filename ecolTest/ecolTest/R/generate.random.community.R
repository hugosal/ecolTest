...
#' @description This function generates a random community sample dataset that satisfies user-specified characteristics.

#' @details This function generates random community sample data: a numeric vector whose elements represent the number of individuals of a species in a sample. The generated sample data is composed of a number of species with a total number of individuals set by the user, such that the community has a Shannon's diversity index approximately equal to a user-specified argument.
...
#' @param silent Logical. Indicates if convergence success or failure messages are omitted. Defaults to FALSE

#' @return A list containing the following components:

#' \itemize{
#'   \item community: If convergence is successful, a numeric vector with the abundance of each species, else \emph{NA}
#'   \item n_ite: If convergence is successful, the number of iterations used to achieve convergence, else \emph{NA}
#' }
...
generate.random.community<-function(H_index, shannon.base=exp(1), sp_num, ntotal, tol=0.0001, maxiter=100, silent=F){
  ...
  cost<-function(comun){return(abs(H_index-sum(comun*log(comun,shannon.base))))}
  iter<-0
  while(abs(H_index - sum(community*log(community,shannon.base),na.rm = T))>tol){
